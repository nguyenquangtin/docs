...
If you need more information on the converter and mapping in general, take a look at [converter, mapping and deltas concept](convert-and-mapping.md).
...
You only need to implement the `supports` method and specify the right `Definition` in `migration_assistant_extension.xml`. The logic to write the data is defined in the `AbstractWriter` class and should almost always be the same. Take a look at [writer concept](writer.md) for more information.
