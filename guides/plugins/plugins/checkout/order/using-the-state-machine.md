...
For each example you have to inject the `Shopware\Core\System\StateMachine\StateMachineRegistry` into your service using the [Dependency Injection container](dependency-injection.md).
...
The following will show an example how to set the order delivery state to "shipped" by just knowing the order ID. For this section, we're going to deal with entity repositories, like explained in our guide about [reading data](reading-data.md).
...
So this example is making use of the repository for the `order_delivery` entity. It was injected previous using the [Dependency injection container](dependency-injection.md) and its respective ID `order_delivery.repository`.
