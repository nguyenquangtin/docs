...
A reverse http cache is a cache server placed before the web shop. If you are not familiar with http caching, please see our [http cache concept](http_cache.md). The reverse http cache needs the following capabilities to fully function with Shopware:
